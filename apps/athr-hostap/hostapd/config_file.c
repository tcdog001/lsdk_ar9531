...
/*
 * nai_realm_list expects the following encoding:
 * <encoding=xx><realm=realm1;[realm=realm2;]>[<eap_method=zz>
 * <auth_id=pp><auth_val=qq>[<auth_id=xx><auth_val=yy>]]
 */
static int parse_nai_realm_list(struct hostapd_bss_config *bss, char *pos,
				int line)
{
	char *start = pos, *end, *tok_end;
	u8 encoding, eap_method, auth_id, auth_val;
	size_t i, num_loops = 0, num_realms = 0;
	u16 realm_len;
	struct hostapd_nai_realm_data *realm = NULL, *realm_new;

	realm = os_zalloc(sizeof(struct hostapd_nai_realm_data));
	if (realm == NULL)
		return -1;

	while ((end = os_strchr(start, '='))) {
		if (num_loops++ > 100) {
			wpa_printf(MSG_ERROR, "Line %d: Invalid "
				   "nai_realm_list substr: %s", line, start);
			goto fail;
		}

		if (os_strncmp(start, "encoding", 8) == 0) {
			if (hexstr2bin(end + 1, &encoding, 1)) {
				wpa_printf(MSG_ERROR, "Lined %d: expecting "
					   "hexstr in encoding=%s",
					   line, end + 1);
				goto fail;
			}
			start += 8 + 1 + 2;
		...
		if (os_strncmp(start, "realm", 5) == 0) {
			u16 realm_buflen;
			if (num_realms == MAX_NAI_REALMS) {
				wpa_printf(MSG_ERROR, "Line %d: Max realms "
					   "(%d) exceeded",
					   line, MAX_NAI_REALMS);
				goto fail;
			}
			tok_end = os_strchr(end + 1, ';');
			if (tok_end == NULL)
				goto fail;
			realm_len = tok_end - end;
			/* realm_buf stores null terminated realm strings */
			for (i = 0, realm_buflen = 0; i < num_realms; i++)
				realm_buflen += os_strlen(realm->realm[i]) + 1;
			if (realm_buflen + realm_len > MAX_NAI_REALMLEN) {
				wpa_printf(MSG_ERROR, "Line %d: Max realmlen "
					   "(%d) exceeded",
					   line, MAX_NAI_REALMLEN);
				goto fail;
			}
			realm->realm[num_realms] =
				realm->realm_buf + realm_buflen;
			os_strncpy(realm->realm[num_realms], end + 1,
				   tok_end - end - 1);
			start = tok_end + 1;
			num_realms++;
			continue;
		...
		if (os_strncmp(start, "eap_method", 10) == 0) {
			if (hexstr2bin(end + 1, &eap_method, 1)) {
				wpa_printf(MSG_ERROR, "Line %d: expecting "
					   "hexstr in eap_method=%s",
					   line, end + 1);
				goto fail;
			}
			start += 10 + 1 + 2;
		...
		if (os_strncmp(start, "auth_id", 7) == 0) {
			...
			if (auth_id != 2 && auth_id != 3 && auth_id != 5 &&
			    auth_id != 6) {
				wpa_printf(MSG_ERROR, "Line %d: auth_id (%d)"
					   "not yet supported", line, auth_id);
				goto fail;
			}
			start += 7 + 1 + 2;

			end = os_strchr(start, '=');
			if (!end || os_strncmp(start, "auth_val", 8) != 0) {
				wpa_printf(MSG_ERROR, "Line %d: expecting "
					   "auth_val to follow auth_id %s",
					   line, start);
				goto fail;
			}
		...
		wpa_printf(MSG_ERROR, "Line %d: unexpected nai_realm_list "
			   "substr: %s", line, start);
		goto fail;
	}
	...
	realm_new = os_realloc(bss->nai_realm_data,
			       sizeof(struct hostapd_nai_realm_data) *
			       (bss->nai_realm_count + 1));
	if (realm_new == NULL)
		return -1;

	bss->nai_realm_data = realm_new;
	realm_new = &bss->nai_realm_data[bss->nai_realm_count];
	os_memcpy(realm_new, realm, sizeof(struct hostapd_nai_realm_data));
	/* patch the pointers */
	for (i = 0; realm->realm[i]; i++) {
		int offset = realm->realm[i] - realm->realm_buf;
		realm_new->realm[i] = realm_new->realm_buf + offset;
	}
	os_free(realm);
	bss->nai_realm_count++;
	for (i = 0; realm_new->realm[i]; i++) {
		wpa_printf(MSG_DEBUG, "NAI Realm %s added",
			   realm_new->realm[i]);
	}
...
fail:
	wpa_printf(MSG_ERROR, "Line %d: Invalid nai_realm_list '%s'",
		   line, pos);
	os_free(realm);
	return -1;
}
...
#ifdef CONFIG_HS20
static u8 * hs20_parse_venue_name(char *pos, u8 *venue_name_len,
				  u8 *prepend_data, u8 prepend_data_len)
{
	char *tok_start, *tok_prev, *tok_end;
	int slen, num_toks, hex, i;
	u8 *venue_name;
	size_t alloc_len;

	if (os_strncmp(pos, "hex:", 4) == 0)
		hex = 1;
	else if (os_strncmp(pos, "asc:", 4) == 0)
		hex = 0;
	else {
		wpa_printf(MSG_ERROR, "Invalid hs20_operator_friendly_name or "
			   "hs20_venue_name");
		return NULL;
	}
	tok_start = tok_prev = pos + 4;
	if (hex)
		slen = os_strlen(tok_start) / 2;
	else
		slen = os_strlen(tok_start);
	alloc_len = slen + 1 + prepend_data_len;
	if (alloc_len > 255) {
		wpa_printf(MSG_ERROR, "Too long hs20_operator_friendly_name "
			   "or hs20_venue_name");
		return NULL;
	}
	venue_name = os_malloc(alloc_len);
	if (venue_name == NULL)
		return NULL;

	if (prepend_data)
		os_memcpy(venue_name, prepend_data, prepend_data_len);

	num_toks = 1;
	while ((tok_start = os_strstr(tok_start, "\\0"))) {
		num_toks++;
		tok_start += 2;
	}
	tok_start = tok_prev = pos + 4;
	tok_end = tok_start + os_strlen(tok_start);
	*venue_name_len = prepend_data_len;
	for (i = 0; i < num_toks; i++) {
		tok_start = os_strstr(tok_start, "\\0");
		if (!tok_start) /* i == num_toks - 1 */
			tok_start = tok_end;
		if (hex) {
			/* first byte is length */
			*(venue_name + *venue_name_len) = (tok_start -
							   tok_prev) / 2;
			*venue_name_len += 1;
			/* rest is lang code and venue name */
			if (hexstr2bin(tok_prev, venue_name + *venue_name_len,
				       (tok_start - tok_prev) / 2)) {
				wpa_printf(MSG_ERROR, "Invalid "
					   "hs20_operator_friendly_name or "
					   "hs20_venue_name");
				os_free(venue_name);
				return NULL;
			}
			*venue_name_len += (tok_start - tok_prev) / 2;
		} else {
			*(venue_name + *venue_name_len) = tok_start - tok_prev;
			*venue_name_len += 1;
			os_memcpy(venue_name + *venue_name_len, tok_prev,
				  tok_start - tok_prev);
			*venue_name_len += tok_start-tok_prev;
		}
		tok_start += 2;
		tok_prev = tok_start;
	}
			       ...
			       char *buf, char *pos, int line)
{
...

	{
		...
		} else if (os_strcmp(buf, "assoc_sa_query_retry_timeout") == 0)
		{
		...
		} else if (os_strcmp(buf, "hs20_venue_name") == 0) {
			u8 venue_info[2];
			if (os_strlen(pos) < 5 ||
			    hexstr2bin(pos, venue_info, 2)) {
				wpa_printf(MSG_ERROR, "Line %d: Invalid "
					   "hs20_venue_name '%s'", line, pos);
				errors++;
				return errors;
			}
			os_free(bss->hs20_venue_name);
			bss->hs20_venue_name = hs20_parse_venue_name(
				pos + 5, &bss->hs20_venue_name_len, venue_info,
				2);
		} else if (os_strcmp(buf, "hs20_network_auth_type") == 0) {
			u8 auth_type;
			u16 redirect_url_len;
			if (hexstr2bin(pos, &auth_type, 1)) {
				wpa_printf(MSG_ERROR, "Line %d: Invalid "
					   "hs20_network_auth_type '%s'",
					   line, pos);
				errors++;
				return errors;
			}
			...
			bss->hs20_network_auth_type =
				os_malloc(redirect_url_len + 3 + 1);
			if (bss->hs20_network_auth_type == NULL) {
				errors++;
				return errors;
			}
			*bss->hs20_network_auth_type = auth_type;
			WPA_PUT_LE16(bss->hs20_network_auth_type + 1,
				     redirect_url_len);
			if (redirect_url_len)
				os_memcpy(bss->hs20_network_auth_type + 3,
					  pos + 2, redirect_url_len);
			bss->hs20_network_auth_type_len = 3 + redirect_url_len;
		} else if (os_strcmp(buf, "hs20_ipaddr_type_availability") ==
			   0) {
			if (hexstr2bin(pos,
				       &bss->hs20_ipaddr_type_availability, 1))
			{
				wpa_printf(MSG_ERROR, "Line %d: Invalid "
					   "hs20_ipaddr_type_availability "
					   "'%s'", line, pos);
				bss->hs20_ipaddr_type_configured = 0;
				errors++;
				return errors;
			}
			bss->hs20_ipaddr_type_configured = 1;
		} else if (os_strcmp(buf, "hs20_nai_realm_list") == 0) {
			if (parse_nai_realm_list(bss, pos, line) < 0)
				errors++;
		} else if (os_strcmp(buf, "hs20_3gpp_cellular_network") == 0) {
			u8 *tmp;
			size_t slen = os_strlen(pos);
			tmp = os_malloc(slen / 2);
			if (tmp == NULL || (slen & 0x01) ||
			    hexstr2bin(pos, tmp, slen / 2)) {
				wpa_printf(MSG_ERROR, "Line %d: Invalid "
					   "hs20_3gpp_cellular_network '%s'",
					   line, pos);
				os_free(tmp);
				errors++;
				return errors;
			...
			bss->hs20_3gpp_cellular_network = tmp;
			bss->hs20_3gpp_cellular_network_len = slen / 2;
		} else if (os_strcmp(buf, "hs20_domain_name_list") == 0) {
			...
			bss->hs20_domain_name_list_value = domain_list;
			bss->hs20_domain_name_list_len = domain_list_len;
		} else if (os_strcmp(buf, "hs20_operator_friendly_name") == 0)
		{
			os_free(bss->hs20_operator_friendly_name);
			bss->hs20_operator_friendly_name =
				hs20_parse_venue_name(
					pos,
					&bss->hs20_operator_friendly_name_len,
					NULL, 0);
		} else if (os_strcmp(buf, "hs20_wan_metrics") == 0) {
			char *tok_start = pos, *tok_end;
			u8 *wan_metrics;
			int res = 0;
			if (bss->hs20_wan_metrics) {
				os_free(bss->hs20_wan_metrics);
				bss->hs20_wan_metrics = NULL;
			}
			wan_metrics = os_malloc(13);
			if (wan_metrics == NULL) {
				errors++;
				return errors;
			}
			while ((tok_end = os_strchr(tok_start, ':'))) {
				if (os_strncmp(tok_start, "wan_info", 8) == 0)
				{
					res = hexstr2bin(tok_end + 1,
							 wan_metrics, 1);
					tok_start += 8 + 1 + 2;
				} else if (os_strncmp(tok_start, "dl_speed", 8)
					   == 0) {
					res = hexstr2bin(tok_end + 1,
							 wan_metrics + 1, 4);
					tok_start += 8 + 1 + 8;
				} else if (strncmp(tok_start, "ul_speed", 8) ==
					   0) {
					res = hexstr2bin(tok_end + 1,
							 wan_metrics + 5, 4);
					tok_start += 8 + 1 + 8;
				} else if (os_strncmp(tok_start, "dl_load", 7)
					   == 0) {
					res = hexstr2bin(tok_end + 1,
							 wan_metrics + 9, 1);
					tok_start += 7 + 1 + 2;
				} else if (os_strncmp(tok_start, "ul_load", 7)
					   == 0) {
					res = hexstr2bin(tok_end + 1,
							 wan_metrics + 10, 1);
					tok_start += 7 + 1 + 2;
				} else if (os_strncmp(tok_start, "lmd", 3) ==
					   0) {
					res = hexstr2bin(tok_end + 1,
							 wan_metrics + 11, 2);
					tok_start += 3 + 1 + 4;
		...
		} else if (os_strcmp(buf, "hs20_connection_capability") == 0) {
			u8 *conn_cap;
			u8 conn_cap_len;
			conn_cap_len = os_strlen(pos) / 2;
			conn_cap = os_malloc(conn_cap_len);
			if (conn_cap == NULL) {
				errors++;
				return errors;
			}
			if (hexstr2bin(pos, conn_cap, conn_cap_len)) {
				wpa_printf(MSG_ERROR, "Line %d: Invalid "
					   "hs20_connection_capability '%s'",
					   line, pos);
				os_free(conn_cap);
				errors++;
				return errors;
			...
			bss->hs20_connection_capability_len = conn_cap_len;
		} else if (os_strcmp(buf, "hs20_operating_class") == 0) {
			u8 *oper_class;
			u8 oper_class_len;
			oper_class_len = os_strlen(pos) / 2;
