...
void print_type1_res(struct nsp_type1_resp *type1esp)
{
    ...
    if ( respoutputfmt == 0 ) {
        printf("\nTOA                   : %llx",htobe64(type1esp->toa));
        printf("\nTOD                   : %llx",htobe64(type1esp->tod));
        printf("\nRTT                   : %llx",htobe64(type1esp->toa)-htobe64(type1esp->tod));
        for(i = 0; i < numRxChains; i++)
            printf("\nRSSI %d                : %x",i, type1esp->rssi[i]);
        printf("\nSend Rate             : %x",ntohl(type1esp->send_rate));
        printf("\nRecv Rate             : %x",ntohl(type1esp->receive_rate));
...
int prepare_mreq(struct req *req, bdaddr_t *station_mac, bdaddr_t *spoof_mac, int channel)
{
    req->nsphdr.SFD = htons(START_OF_FRAME);
    req->nsphdr.version = NSP_VERSION;
...
int send_req(struct ap *ap, bdaddr_t *station_mac,bdaddr_t *ap_mac, int channel)
{
    int i = 0, readlen = 0;
    ...
    prepare_mreq(&ap->req, station_mac, ap_mac, channel);
