                ...
                if(FD_ISSET(i, &readset)) {
                    ...
                    if (readcount != resplen) {
                        ...
                        if( frame_type ==  NSP_TYPE1RESP ) {
                            ...
                            if(ntohs(mres->result) == 0 || ntohs(mres->result) == 1 || (ntohs(mres->result) == 4 && ntohs(mres->no_of_responses) !=0)) {
...
void usage()
{
    ...
    printf("\t-p, --packettype        Packet Type. Specifies the packet type to be used. Null(0). Default QosNULL(1). RTS/CTS (2)\n");
...
int main(int argc, char *argv[])
{
    ...
    transmit_mode = 5;
    while((opt=getopt_long(argc, argv, "avc:g:T:drowhsSPD:WCi:t:k:IR:p:m:n:", main_options, NULL)) != -1)
    {
        switch (opt) {
    ...
    while (1) {
        ...
        for( j=0; j<no_of_stations; j++) {
            ...
            for (i=0; i<no_of_aps; i++) {
                if(wakeup_station) {
                    if(!station[j].result) {
                        send_req(&aps[i], &station[j].station_mac,&station[j].ap_mac,station[j].station_channel);
        ...
        // TBD: this change is to test concurrencey 
        mreqcnt = 0;
        // Just giving wait time before starting the next cycle of seq. of reuqests. 
        usleep(200*10000);
        }
    }
}
