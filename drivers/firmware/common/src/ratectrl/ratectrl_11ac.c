            ...
            struct TxRateCtrl_s  *pRc, A_UINT8 bw, A_UINT8 pastRate)
{
        ...
        if (RT_IS_HT_INDEX(maxRate)) {
            ...
            for (nss = NUM_SPATIAL_STREAM - 1; nss >= 0; --nss) {
                mcs = max_mcs - (index - pRc->u.tD.rateMax[bw][nss]);
                index = pRc->u.tD.rateMax[bw][nss];
                ...
                for (; mcs >= 0; --mcs, --index) {
                    ...
                    if (100 * RT_GET_USER_KBPS(pRateTable, index) < bestThruput[nss]) {
                        /*
                    ...
                    if (bestThruput[nss] <= thisThruput) {
                        bestThruput[nss] = thisThruput;
                        bestRate[nss]    = index;
                        if (bestThruput[nss] >= bestThruput[bestNssIdx]) {
                            bestNssIdx = nss;
                        }
                    }
                }
...
#endif
{
        ...
        rc_mask_info->idx != WHAL_RC_MASK_IDX_NON_HT)
    {
        ...
        for (i = nss_idx - 1; i < NUM_SPATIAL_STREAM; ++i) {
            ...
            if (nowMsec > pRc->u.tD.probeTime[index][i] +
                pRc->u.tD.probePeriod[index][i] && 
                pRc->u.tD.probeTime[index][i] < minProbeTime)
            {
                ...
                if (nextRate != RT_INVALID_INDEX) {
                    minProbeTime = pRc->u.tD.probeTime[index][i];
                    *probe_rix = nextRate;
                    probeNssIdx = i;
                } else {
                    /* couldn't find a good rate, delay probe */
                    pRc->u.tD.probeTime[index][i] = nowMsec;
                }
            }
        }
