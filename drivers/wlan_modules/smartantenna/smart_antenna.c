...
void sa_set_train_bw(struct sa_node_ccp *node)
{
    ...
    SA_DEBUG_PRINT(SA_DEBUG_LVL2,node->node_info.radio_id,"%s %d %d %d \n",__func__,nppdu_bw[SA_BW_20],nppdu_bw[SA_BW_40],nppdu_bw[SA_BW_80]);
}
...
int32_t sa_update_ratestats(struct sa_node_ccp *node, struct sa_tx_feedback *feedback)
{
    ...
    while (num_fb) {
        ...
        if(IS_SA_11B_RATE(rate)) {
            ...
            SA_DEBUG_PRINT(SA_DEBUG_LVL3,node->node_info.radio_id,"%s 11B: bw = %d nss = %d rate = %d mcs = %d series %d index %d \n",__func__, bw, nss, rate, mcs, series, index);
            node->rate_stats.npackets_legacy[index] += npkts;
        } else if (IS_SA_11G_RATE(rate)) {
            /* 11g rate code */
            index = SA_STAT_INDEX_11G(rate);
            SA_DEBUG_PRINT(SA_DEBUG_LVL3,node->node_info.radio_id,"%s, 11G/A: bw = %d nss = %d rate = %d mcs = %d series %d index %d \n",__func__, bw, nss, rate, mcs, series, index );
            node->rate_stats.npackets_legacy[index] += npkts;
        } else {
...
void sa_cancel_train_params(struct sa_node_ccp *node, uint32_t antenna)
{
    ...
    SA_DEBUG_PRINT(SA_DEBUG_LVL2,node->node_info.radio_id,"%s train antenna %d, rate %x, numpkts %d\n",__func__,train_data->antenna,train_data->ratecode,train_data->numpkts);
...
uint32_t sa_select_rx_antenna(struct sa_node_ccp *node,struct radio_config *radio)
{
    ...
    if (!radio->init_config.bss_mode) {
        ...
        if (train_info->previous_selected_antenna != train_info->selected_antenna) {
            ...
            if ((train_info->selected_antenna != radio->default_antenna) &&
                ...
                SA_DEBUG_PRINT(SA_DEBUG_LVL1,node->node_info.radio_id,"%s: receive antenna selection error\n",__func__);
            }
        }
    ...
    SA_DEBUG_PRINT(SA_DEBUG_LVL2,node->node_info.radio_id,"%s: receive antenna %d selected ant %d pselected antenna %d\n",__func__, radio->default_antenna, train_info->selected_antenna,train_info->previous_selected_antenna);
...
uint32_t sa_check_bw_change(struct sa_node_ccp *node)
{
    ...
    SA_DEBUG_PRINT(SA_DEBUG_LVL2,node->node_info.radio_id,"%s %d %d %d",__func__,node->train_info.nppdu_bw[SA_BW_80],node->train_info.nppdu_bw[SA_BW_40],node->train_info.nppdu_bw[SA_BW_20]);
    ...
    if (((elapsed_time >= sa_params->retrain_interval) && (train_enable & SA_PERIOD_TRAIN_EN))
        ...
        if (elapsed_time >= sa_params->perftrain_interval) {
            ...
            for (bw = 0; bw < SA_MAX_BW; bw++) {
                ...
                SA_DEBUG_PRINT(SA_DEBUG_LVL2,node->node_info.radio_id, "BW %d Avg Goodput : %d : Ins Goodput %d nppdu %d hysteresis: %d\n",bw,
...
int __sa_update_txfeedback(void *ccp, struct sa_tx_feedback *feedback, uint8_t *status)
{
    ...
    if (node->train_info.smartantenna_state == SMARTANTENNA_PRETRAIN) {
        ...
        if (node->train_info.txed_packets >= sa_params->num_pretrain_packets) {
            *status |= SMART_ANT_TRAINING_REQUIRED;
            SA_DEBUG_PRINT(SA_DEBUG_LVL2,node->node_info.radio_id,"PRE TRAINING COMPLETED\n");
        }
        ...
        if (feedback->is_trainpkt) {
            ...
            if (check_bw_change) {
                SA_DEBUG_PRINT(SA_DEBUG_LVL2,node->node_info.radio_id,"train pkt with bw mismatch %d\n",node->train_info.nppdu);
...
int  __sa_get_training_info(void *ccp , uint32_t *rate_array, uint32_t *antenna_array, uint32_t *numpkts)
{
    ...
    if(node->train_info.smartantenna_state == SMARTANTENNA_PRETRAIN) {
        ...
        SA_DEBUG_PRINT(SA_DEBUG_LVL2,node->node_info.radio_id,"%s initial rate code 0x%x, bw %d\n",__func__,ratecode, train_info->train_state.bw);
        ...
        SA_DEBUG_PRINT(SA_DEBUG_LVL2,node->node_info.radio_id,"%s re init rate code 0x%x, bw %d\n",__func__,ratecode, train_info->train_state.bw);
        sa_set_train_params(node, train_info->train_state.ant_map[0], train_info->train_state.rate);
...
int sa_init_antmap(struct sa_node_ccp *ccp)
{
    ...
    for (i = 0; i < sa_params->num_tx_antennas; i++) {
        SA_DEBUG_PRINT(SA_DEBUG_LVL1,ccp->node_info.radio_id,"%s Mac: %s RadioId: %d ant_map[%d] = %d\n",
...
int __sa_node_connect(void  **ccp, struct  sa_node_info *node_info)
{
    ...
    *ccp = node;
    ...
    SA_DEBUG_PRINT(SA_DEBUG_LVL1,node->node_info.radio_id,"%s Mac: %s RadioId: %d max bw: %d chainmask: %d opmode: %d  channel : %d\n",
