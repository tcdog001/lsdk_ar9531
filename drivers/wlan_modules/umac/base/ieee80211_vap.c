                ...
                u_int8_t              *mataddr)
{
    ...
    /*                                                                      
     * Ensure that beacon interval is atleast IEEE80211_OL_BINTVAL_MIN times
     * the number of AP VAPs in multi-VAP configuration                                
     */                                                                     
    if (opmode == IEEE80211_M_HOSTAP) {
        ic->ic_num_ap_vaps++;
        if (ic->ic_is_mode_offload(ic) && (ic->ic_num_ap_vaps > 1) && 
            (ic->ic_intval < (ic->ic_num_ap_vaps * IEEE80211_OL_BINTVAL_MIN))){
            ic->ic_intval = ic->ic_num_ap_vaps * IEEE80211_OL_BINTVAL_MIN;
...
wlan_vap_delete(wlan_if_t vaphandle)
{
    ...
    if (ieee80211_vap_deleted_is_clear(vap)) /* if not deleted then it is on the list */
    {
        ...
        if (vap->iv_opmode == IEEE80211_M_HOSTAP) {
            if (ic->ic_is_mode_offload(ic) && (ic->ic_num_ap_vaps > 1) && 
                (ic->ic_intval == (ic->ic_num_ap_vaps * IEEE80211_OL_BINTVAL_MIN))){
                ic->ic_intval -= IEEE80211_OL_BINTVAL_MIN;
                ic->ic_need_vap_reinit = 1;
            }
            ic->ic_num_ap_vaps--;
        }
