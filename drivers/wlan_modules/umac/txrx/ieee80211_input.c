...
ieee80211_input_data(struct ieee80211_node *ni, wbuf_t wbuf, struct ieee80211_rx_status *rs, int subtype, int dir)
{
    ...
    if (subtype == IEEE80211_FC0_SUBTYPE_NODATA || subtype == IEEE80211_FC0_SUBTYPE_QOS_NULL) {
        ...
        ic->tdls_recv_null_data(ic, ni, wbuf);
#endif
...
#define IEEE80211_HANDLE_MGMT(_ic, _vap, _ni, _wbuf, _subtype, _type, _rs) \
        if (_vap->iv_input_mgmt_filter == NULL || \
             ...
             */                                             \
            ieee80211_recv_mgmt(_ni, _wbuf, _subtype, _rs);     \
        }                                                   \
        /*                                                  \
...
#define IEEE80211_HANDLE_MGMT( _ic, _vap, _ni, _wbuf, _subtype, _type,  _rs) \
      ...
      {                                                                                         \
            {                                                                                   \
                ...
                temp_data = (ieee80211_recv_mgt_args_t *)adf_nbuf_get_priv(_wbuf);               \
                adf_os_mem_zero(temp_data,sizeof(struct ieee80211_recv_mgt_args));              \
                temp_data->ni = _ni;                                                             \
                temp_data->subtype = _subtype;                                                   \
                adf_nbuf_queue_add(&_ic->ic_mgmt_nbufqueue, _wbuf);                               \
...
#define HAS_SEQ(type, subtype)   (((type & 0x4) == 0) && ((type | subtype) != QOS_NULL))
    ...
    if ((ieee80211_scan_in_home_channel(ic->ic_scanner)) ||
...
#if CONFIG_RCPI
        ATH_RSSI_LPF(ATH_NODE_NET80211(ni)->an_avgrssi, rs->rs_rssi);
#endif /* CONFIG_RCPI */
        ...
        /* 
         * Update the sequence number of the valid data frames     
         */
    ...
    ieee80211_tdls_pwrsave_check(ni, wh);
    ...
    if (type == IEEE80211_FC0_TYPE_DATA) {
        ...
        /*
         * Modify the Channel Switch state machine so that the TDLS STA receives 
         * at least 1 beacon once it enters the base channel. There are cases in which 
         * the TDLS STA is consistently late in returning to the base channel to received 
         * the DTIM beacon, then it leaves the base channel before the next beacon arrives. 
         * This can lead to BMISS indication and other problems.
         * to avoid deadlocks, a parameter specifies the maximum amount of time the 
         * Channel Switch state machine waits for the beacon.
         */
...
{
    int igmp = 0;
#if defined (ATH_SUPPORT_HYFI_ENHANCEMENTS) || defined (ATH_SUPPORT_AOW)
#if ATH_SUPPORT_HYFI_ENHANCEMENTS || ATH_SUPPORT_AOW
    struct ieee80211com *ic= vap->iv_ic;
#endif
#endif
    if (!IEEE80211_VAP_IS_DELIVER_80211_ENABLED(vap)) {
        ...
        if (vap->iv_txrx_event_info.iv_txrx_event_filter & (IEEE80211_VAP_INPUT_EVENT_UCAST | IEEE80211_VAP_INPUT_EVENT_EOSP)) {
            ...
            if (vap->iv_txrx_event_info.iv_txrx_event_filter & IEEE80211_VAP_INPUT_EVENT_EOSP && 
                ...
                if(((struct ieee80211_qosframe *)wh)->i_qos[0] & IEEE80211_QOS_EOSP) {
                    ...
                    /*
                     * TDLS Peer U-APSD needs to filter events using the node structure.
                     * Thus, forward the node pointer in the vap txrx event.
                     */
                    evt.ni = ni;
                    ieee80211_vap_txrx_deliver_event(vap,&evt);
                }
...
int ieee80211_input_sta(struct ieee80211_node *ni, wbuf_t wbuf,struct ieee80211_rx_status *rs)
{
    ...
    if ((ieee80211_vap_ready_is_set(vap)) &&
        ...
        if ((vap->iv_bmiss_count > 0) && !IEEE80211_IS_TDLS_NODE(ni)) {
#ifdef ATH_SWRETRY                     
