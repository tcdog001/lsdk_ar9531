...
extern void ap_usb_host_led_on(int gpio);
extern void ap_usb_host_led_off(int gpio);
#endif
...
#endif /* CONFIG_MACH_AR934x or CONFIG_MACH_QCA955x or CONFIG_MACH_QCA956x */
...
void usb_disconnect(struct usb_device **pdev)
{
	...
	ap_usb_led_off();
     ...
     if(!(udev->bus->controller->driver->mod_name)){
        ap_usb_host_led_off(AP_USB1_LED_GPIO);
     }else{
        ap_usb_host_led_off(AP_USB2_LED_GPIO);
     }
#endif
...
#endif /* CONFIG_MACH_QCA955x or CONFIG_MACH_QCA956x */
		...
		int retry_counter)
{
    ...
    defined(CONFIG_MACH_QCA956x)
	/*
	 * Scorpion/HoneyBee/Dragonfly Specific
	 * PATCH for USB
	...
	if(!(udev->bus->controller->driver->mod_name)) {
		...
		/*
		 * Host controller 2. Only for Scorpion and Dragonfly. Not for Honeybee.
		 */
...
#endif /* CONFIG_MACH_QCA955x or CONFIG_MACH_QCA953x or CONFIG_MACH_QCA956x */
...
#if defined(AP_USB1_LED_GPIO) || defined(AP_USB2_LED_GPIO)
         if(!(udev->bus->controller->driver->mod_name)){
            ap_usb_host_led_on(AP_USB1_LED_GPIO);
         }else{
            ap_usb_host_led_on(AP_USB2_LED_GPIO);
         }
#endif
